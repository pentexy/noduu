import asyncio import logging import re import time import psutil import os import requests from datetime import datetime from telethon import TelegramClient, events from telethon.tl.functions.messages import GetDialogsRequest from telethon.tl.types import InputPeerEmpty from telethon.tl.types import MessageEntityBold
...
async def type_and_send(event, message, **kwargs): async with client.action(event.chat_id, "typing"): await asyncio.sleep(0.7) await event.reply(message, **kwargs)
...
@client.on(events.NewMessage(pattern=r"/pm (on|off)")) async def toggle_pm(event): user_id = event.sender_id state = event.pattern_match.group(1) user_flags[user_id] = state await event.reply(f"ᴘʀɪᴠᴀᴛᴇ ᴍᴇssᴀɢᴇs {state} ғᴏʀ ʏᴏᴜ.")
